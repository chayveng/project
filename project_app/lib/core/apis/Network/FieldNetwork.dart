...
class FieldNetwork {
  ...
  static Future<ApiResponse> getByClubId({@required int clubId}) async =>
      apiResponseFromJson(
          await ApiConnect.get(path: '/field/getByClubId/$clubId'));

  static Future<ApiResponse> add({@required Field field}) async =>
      apiResponseFromJson(
          await ApiConnect.post(path: '/field/add', body: field.toJson()));

  static Future<ApiResponse> delete({@required int id}) async =>
      apiResponseFromJson(
          await ApiConnect.post(path: '/field/delete/$id', body: null));
